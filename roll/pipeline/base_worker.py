...
class ActorWorker(Worker):
    ...
    def stop_server(self, data: DataProto = None):
        ...
        # Here, we should notify the scheduler this generation step is finished,
        # reser the worker to pending status.
        shared_storage = self.strategy.shared_storage
        if shared_storage is not None:
            shared_storage.set(f"{self.worker_name}_status", "pending")
        else:
            print("*** ActorWorker: no shared_storage found!")
