...
class ActorWorker(Worker):
    ...
    def _strategy_server_wrapper(self,
        ...
        if self.pipeline_config.enable_migration:
            strategy.ready = True
    ...
    def start_server(self, data: DataProto):
        ...
        """
        print(f"infer-worker {self.rank}.start_server")
        global_step = data.meta_info.get("global_step", 0)
        is_offload_states = data.meta_info.get("is_offload_states", True)
    ...
    def request_complete(self, data: DataProto):
        ...
        try:
            ...
            # TODO: we don't know why keyerror happens. Forcely applying the callback function causes another error.
            self.logger.warning(f"request_id {data.meta_info['request_id']} not found in response_call_back_fns")
