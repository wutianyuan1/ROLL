    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer:
                ...
                ## HACK (Madoka): remove validation to improve efficiency 
            ...
            ## HACK (Madoka): remove checkpointing to improve efficiency
...
def get_traj_rollout_time(batch: DataProto) -> torch.Tensor:
    ...
    scores = []
    for traj_id,  traj_batch in batch_group_by_traj.items():
        episode_scores = traj_batch.non_tensor_batch["traj_rollout_time"][0] if "traj_rollout_time" in traj_batch.non_tensor_batch else [0.0]
        scores.append(episode_scores)
    return torch.tensor(scores, dtype=torch.float32)
...
def compute_data_metrics(batch):
    ...
    # traj_rollout_times = []
    try:
        print(f"[batch.batch.values]{batch.batch.values()}")
        print(f"[batch.meta_info.values]{batch.meta_info.get('metrics', {}).values()}")
        print(f"[batch.meta_info.metrics]{batch.meta_info.get('metrics', {}).values()}")
    except Exception as e:
        print(f"[ERROR IN AGENTIC PIPELINE][compute_data_metrics]{e}")
