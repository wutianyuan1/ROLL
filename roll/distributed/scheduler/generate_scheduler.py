...
class DynamicSamplingScheduler:
    ...
    def __init__(self, pipeline_config=None, job_name='default'):
        ...
        # min_concurrency_ratio is only used when migration is enabled.
        # The generation starts iff #ready workers >= min_concurrency_ratio * #total workers,
        self.min_concurrency_ratio = float(os.environ.get("RATIO", 1.0))
    ...
    def get_batch(self, data: DataProto, batch_size: int) -> DataProto:
        ...
        migration_scheduler: MigrationSchedulerBase =\
            StaticAggMigrationScheduler([3], 16)
        ...
        while True:
            ...
            # When migration is enabled, start generating when #ready workers >= r * #total workers (0 < r <= 1).
            # when migration is disabled, the workers will be ready simutaneously, so we just check whether #ready workers > 0.
            # Because the strategy.ready is set to True after barrier in VllmStrategy.start_server.
            if self.enable_migration:
                can_start = len(ready_workers) >= self.min_concurrency_ratio * len(self.actor_cluster.workers)
            else:
                can_start = len(ready_workers) > 0
            if not can_start:
                if len(worker_status) != 0:
                    if self.enable_migration:
                        print(f"*** No enough workers, expect at least {self.min_concurrency_ratio} * {len(self.actor_cluster.workers)}, current status: {worker_status}")
                    else:
                        print(f"*** No enough workers, expect all {len(self.actor_cluster.workers)}, current status: {worker_status}")
                time.sleep(1)
                continue
            ...
            migrate_src_dst = {}
            if self.enable_migration:
                migrate_src_dst = migration_scheduler.migrate(get_worker_stat_fns, self.request_id_2_dp_rank, ready_workers)
            if len(migrate_src_dst) != 0:
                ...
                for message in pubsub_channel.listen():
                    ...
                    for key in self.shared_storage.scan_iter(match=f'{self.job_name}:token_ids_{src_rank}_{dst_rank}*'):
                        ...
                        # New request should have max_new_tokens = max_new_tokens - #already generated tokens.
                        new_req.meta_info["generation_config"]["max_new_tokens"] -= len(token_latencies)
                    ...
                    if processed_migrations >= len(migrate_src_dst):
                        break
                for worker_id in range(len(self.actor_cluster.workers)):
                    if worker_id not in migration_scheduler.dest_workers:
                        worker_stats: EngineStats = ray.get(self.actor_cluster.workers[worker_id].get_stats.remote())
                        unfinished_count = worker_stats.num_running_reqs + worker_stats.num_swapped_reqs + worker_stats.num_waiting_reqs
                        assert unfinished_count == 0
                        print(f"==== Call stopserver to worker {worker_id} due to no active requests on it, current engine stats: {worker_stats}")
                        self.actor_cluster.workers[worker_id].stop_server.remote()
                with self.lock:
                    origin_req_ids = set()
