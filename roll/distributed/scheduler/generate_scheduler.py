...
class DynamicSamplingScheduler:
    ...
    def get_batch(self, data: DataProto, batch_size: int) -> DataProto:
        ...
        while True:
            ...
            # For non-skip iterations, we update running_workers based on the new worker_status.
            if len(worker_status) != 0:
                print(f"=== alive check: {worker_status}")
                running_workers = [i for i in range(len(worker_status)) if worker_status[i] == "running"]
            ...
            dp_rank = self.get_available_dp_rank_no_wait(running_workers)
    ...
    def get_available_dp_rank_no_wait(self, running_workers):
        # 负载均衡逻辑，期望各dp 正在处理的条数基本接近
        sorted_ranks = sorted(
            self.load_balance_coordinator.keys(), key=lambda rank: (self.load_balance_coordinator[rank], rank)
        )
        for rank in sorted_ranks:
            if (rank in running_workers) and (self.load_balance_coordinator[rank] < self.max_running_requests):
                return rank
        return None
