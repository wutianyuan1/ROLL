        ...
        for da in da_2_num_gpus_per_node:
            ...
            for node in ray_nodes:
                ...
                for key in resource.keys():
                    ...
                    # The reason we made this change is that for some device affinity like 'NVIDIA H20', the key is 'accelerator_type:H20',
                    # then da '==' key cannot hold. So we use 'startswith'.
                    if key.startswith(DEVICE_TYPE_STR) and da.startswith(f"NVIDIA {key[len(DEVICE_TYPE_STR):]}") and node_gpu_num >= da_2_num_gpus_per_node[da]:
