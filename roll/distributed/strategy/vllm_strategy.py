...
class VllmStrategy(InferenceStrategy):
    ...
    def __init__(self, worker: Worker):
        ...
        self.request_metas = {}
        self.req_id_2_resp_start_idx = {} # Here req_id is id of the original request that has response(s) to migrate
        self.req_id_2_token_latencies = {} # Here req_id is id of the migrated response
    ...
    def process_vllm_output(self, vllm_outputs: List[RequestOutput], request_complete_callback):
        ...
        for origin_req_id, resps in origin_req_id_2_resps.items():
            ...
            if len(resps) + len(keys) == origin_req_id_2_num_seqs[origin_req_id]:
                ...
                output_token_ids = []
                output_token_latencies = []
                assert origin_req_id in self.request_metas
                ttft = {} # We can derive TTFT of each response in the same way
                tpot = {} # TODO: Lunxi: Does TPOT include the token generated by re-prefill?
                re_prefill = {} # Only migrated responses have this metric
                tmax = {}
                lens = {}
                # Collect cached responses.
                for key in keys:
                    # For token_ids
                    resp_id = key.split("_")[4]
                    ...
                    output_token_latencies.append(token_latencies_list[1:])
                    # Consistency check
                    assert len(output_token_ids[-1]) == len(output_token_latencies[-1]), \
                        f"#token_ids ({len(output_token_ids[-1])}) does not match #token_latencies ({len(output_token_latencies[-1])})"
                    ...
                    lens[resp_id] = len(output_token_latencies[-1])
                ...
                for resp_id, resp in resps:
                    ...
                    # For token_latencies
                    previous_token_latencies = self.req_id_2_token_latencies[resp.request_id]
                    output_token_latencies.append(previous_token_latencies + list(resp.outputs[0].token_latencies))
                    # Consistency check
                    assert len(output_token_ids[-1]) == len(output_token_latencies[-1]), \
                        f"#token_ids ({len(output_token_ids[-1])}) does not match #token_latencies ({len(output_token_latencies[-1])})"
                    # print(f"=== migrated request {origin_req_id}'s response {resp_id} #token_ids: {len(output_token_ids[-1])} #token_latencies: {len(output_token_latencies[-1])}")
                ...
                print("TTFT      |", {k: f'{v:.3f}'for k, v in ttft.items()})
                print("TPOT      |", {k: f'{v:.3f}'for k, v in tpot.items()})
                print("Re-Prefill|", {k: f'{v:.3f}' if v is not None else v for k, v in re_prefill.items()}),
                print("Tmax      |", {k: f'{v:.3f}'for k, v in tmax.items()})
                print("Len       |", lens)
                # assume num_return_sequenses == 1
                if len(lens) == 1:
                    with open("req_complete_t_and_length.out", 'a') as f:
                        f.write(f"{origin_req_id} {list(lens.values())[0]} {time.time() - self.worker.worker_start_time}\n")
        ...
        for request_output in normal_reqs:
            ...
            if request_id not in self.request_metas:
                continue
            for resp_id, completion_output in enumerate(request_output.outputs):
                # For token_ids
                output_token_ids.append(completion_output.token_ids)
                # For token_latencies
                output_token_latencies.append(list(completion_output.token_latencies))
                # Consistency check
                assert len(output_token_ids[-1]) == len(output_token_latencies[-1]), \
                    f"#token_ids ({len(output_token_ids[-1])}) does not match #token_latencies ({len(output_token_latencies[-1])})"
                # print(f"=== unmigrated request {request_id}'s response {resp_id} #token_ids: {len(output_token_ids[-1])} #token_latencies: {len(output_token_latencies[-1])}")
            ...
            lens = [len(resp_latencies) for resp_latencies in output_token_latencies]
            ...
            if len(lens) == 1:
                with open("req_complete_t_and_length.out", 'a') as f:
                    f.write(f"{request_id} {lens[0]} {time.time() - self.worker.worker_start_time}\n")
            request_complete_callback(data=output_data)
        assert len(normal_req_ids_set) == 0

    def start_server(self, data: DataProto, request_complete_callback):
        if not self.worker.pipeline_config.enable_migration:
        ...
        while True:
            while not self.command_queue.empty():
                ...
                if command == GenerateRequestType.ADD:
                    ...
                    # while the latter has additional fields including 'origin_request_id', 'token_latencies', and 'resp_start_idx',
                    ...
                    if "origin_request_id" in batch.meta_info:
                        ...
                        if origin_req_id not in self.request_metas:
                            ...
                            origin_req_meta_info.pop("token_latencies")
                    ...
                    if 'resp_start_idx' in batch.meta_info:
                        ...
                        assert batch.meta_info['request_id'] not in self.req_id_2_token_latencies
                        self.req_id_2_token_latencies[batch.meta_info['request_id']] = batch.meta_info['token_latencies']
                    ...
                    if batch.meta_info["role"] == "src":
                        ...
                        for req_output in reqs_to_migrate:
                            ...
                            for resp_id, output_response in enumerate(req_output.outputs):
                                assert len(output_response.token_ids) == len(output_response.token_latencies)
                    ...
                    self.req_id_2_token_latencies.clear()
                    while not self.command_queue.empty():
