...
class DockerRuntime(ExecutionEnvironment):
    ...
    def start_container(self):
        ...
        """
        self.retry_times = 0
        RETRY_TIMES = 100 # TODO：这里当前服务比较稳定，try逻辑改到docker里
        clear_time = 60 # TODO 
        ...
        while self.retry_times < RETRY_TIMES:
            try:
        ...
        self.logger.error(f"[{os.getpid()}][START CONTAINER ERROR](retry_times: {self.retry_times}) docker_image: {self.docker_image}, route_key: {self.route_key}")
        return 'ERROR'
    ...
    async def stop_container(self):
        try:
            ...
            self.logger.error(f"[{os.getpid()}][STOP CONTAINNER ERROR] route_key: {self.route_key}, container_name: {self.container_name}, error message: {repr(e)}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    self.logger.warning(f"Container stop request failed with status: {response.status}")
    ...
    def execute(self, command: str | list[str], workdir: str, environment: dict, timeout=180) -> str:
        ...
        while retry_times < 10:
            ...
            retry_times += 1
            self.logger.info(f'[EXCUTE FAILED](RETRY_times:{retry_times}) execute command: {command}, container_name: {self.container_name}, timeout: {timeout}')

        self.logger.error(
            f"[EXECUTE ERROR](retry_times:{retry_times}) "
            f"route_key: {self.route_key}, input: {[data]}, error: {error_message}"
        )
        ...
        while i < MAX_RETRY:
            try:
                with open(src_path, 'rb') as local_file:
                    ...
                    response = requests.post(f'{self.host}/upload',
                                            headers={"ROUTE-KEY": self.route_key},
                                            data=data,
                                            files=files)
                i += 1
                ...
                # self.logger.error(f'[COPY TO CONTAINER ERROR] route_key: {self.route_key}, data: {data}, files: {files}, error: {repr(e)}')
                pass
        self.logger.error(f'[COPY TO CONTAINER ERROR](retry_time:{i}) route_key: {self.route_key}, data: {data}, files: {files}, error: {repr(e)}')
    ...
    def create_file(self, file_path: str, content: str):
        ...
        while i < MAX_RETRY:
            try:
                response = requests.post(f'{self.host}/write_file',
                                        headers={"ROUTE-KEY": self.route_key},
                                        json=data)
                i += 1
                if response.status_code == 200:
                    response_data = response.json()
                    if response_data.get("status") == "Success":
                        # self.logger.info(f'[CREATE FILE SUCCESS] route_key: {self.route_key}, data: {data}')
                        return 
            except requests.exceptions.RequestException as e:
                # self.logger.error(f'[CREATE FILE ERROR] route_key: {self.route_key}, data: {data}, error: {repr(e)}')
                pass
        self.logger.error(f'[CREATE FILE ERROR](retry_time:{i}) route_key: {self.route_key}, data: {data}, error: {repr(e)}')
    ...
    def run(
        ...
        try:
            ...
            # exec_result = self.execute(command=["/bin/sh", "-c", command],
            ...
            # 检查 exec_result 是否为字符串（错误情况）
            if isinstance(exec_result, str): # 执行服务发生了问题
                self.logger.error(f"[服务执行有问题]command: {[command]},exec_result({type(exec_result)}): {exec_result}")
                stdout = exec_result
                if exec_result.startswith('ERROR: '): error_code = -1
                else: error_code = 0
            elif isinstance(exec_result, dict): # 执行成功
                stdout = clean_stdout(exec_result["stdout"] + exec_result["stderr"])
            ...
            if error_code == 124: # 运行超时
                self.logger.error(f"[运行超时]Internal Timeout: {timeout}s")
    ...
    def _rm_conda_in_swebench(self):
        ...
        try:
            ...
            if not out_file:
                self.logger.error("无法读取 /run_tests.sh 文件")
                return False
...
{modified_content}"""
            ...
            self.logger.error(f"注释环境命令时出错: {repr(e)}")
            return False
    ...
    def _backup_and_restore_run_tests(self, action='backup'):
        ...
        try:
            ...
            self.logger.error(f"备份/恢复文件时出错: {repr(e)}")
            return False
    ...
    def _calculate_reward_r2e(self, get_test_output=False) -> float:
        ...
        if not parse or not expected:
            print(f'[ATTENTION][need check]parse or expected is null. parse: {parse}, expected: {expected}')
            return 0.0
        if len(parse) != len(expected):
            ...
            for k in parse.keys():
                if k not in expected:
                    print(f'[ATTENTION][need check]k not in expected. k: {k}, parse: {parse}, expected: {expected}')
                    continue
