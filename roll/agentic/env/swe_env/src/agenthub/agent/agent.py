...
class Agent:
    ...
    def __init__(self, name: str, args: AgentArgs, logger=None):
        ...
        self.infer_mode = self.llm_name.split('/')[0] # choice from ['ideallab', 'whale_openai']
    ...
    def request_litellm(self,model,api_key,base_url,messages,tools,extra_args):
        kwargs = {
            "tool_choice": "none",
            "function_call": None,
        }
        ...
        response = litellm.completion(
            ...
            tools=tools,
            messages=messages,
            timeout=extra_args.get("timeout",self.llm_timeout),
            temperature=extra_args.get("temperature",0),
            api_base=base_url,
            # max_tokens=3000,
            **kwargs,
        )
    ...
    def request_ideallab(self,model,api_key,base_url,messages,tools,extra_args):
        client = openai.OpenAI(api_key=api_key ,base_url=base_url)
        completion = client.chat.completions.create( # TODO: 其他参数支持
                                        model=model, 
                                        messages = messages,
                                        tools = tools,
                                        # temperature=extra_args.get("temperature",0.8),
                                        **extra_args
                                )
        self.logger.info('**************************************')
        self.logger.info(f'\n[input]{messages}')
        ...
        total_tokens = completion.usage.total_tokens # total = prompt + completion
        ...
        return {'response':completion, 'token_cost':token_cost}
    
    def request_whale_openai(self,model,api_key,base_url,messages,tools,extra_args):
        """
        调用whale上的模型, 采用OPENAI协议。
        其他方式参考：https://code.alibaba-inc.com/whale-demo/whale-demo/blob/master/whale-python/openai/OpenAISDKReason.py
        """
        stream = False
        ...
        client = OpenAI(
            api_key=api_key,
            base_url=base_url,# 查阅Whale API 服务文档确认域名 https://aliyuque.antfin.com/gdorir/ahrwo0/gcwgz78cmz15l8r2
        )
        ...
        completion_tokens = completion.usage.completion_tokens
        total_tokens = completion.usage.total_tokens # total = prompt + completion
        if enable_thinking:reasoning_content = completion.choices[0].message.reasoning_content
        else:reasoning_content = ''
        # TODO: 暂时把thinking部分扔掉了
        # response = f'<think>{reasoning_content}</think><result>{response_content}</result>'
    ...
    def model_query(
        ...
        if self.use_fn_calling:
            tools = [search_tool, file_editor, bash_execute_tool, finish_tool] # 总共四种工具
        ...
        while retries < self.max_retries:
            try:
                ...
                if self.infer_mode == 'whale_openai':
                    model_response = self.request_whale_openai(model=self.model_name, api_key=os.environ['WHALE_API_KEY'],base_url=os.environ['WHALE_URL'],messages = messages_,tools=tools,extra_args=kwargs)
                else:
                    model_response = self.request_ideallab(model=self.model_name, api_key=os.environ['OPENAI_API_KEY'],base_url=os.environ['BASE_URL'],messages = messages_,tools=tools,extra_args=kwargs)
                response = model_response['response']
                token_cost = model_response['token_cost']
