...
HYPER_PERIOD_SCHEDULE = [f'job{i}' for i in [1, 2, 3, 4]]
...
class EmulatedJob:
    ...
    def _run_initialization(self):
        ...
        print(f"--- {label}: init begin")
        all_gpus = []
        train_gpus = [i for i in self.train_gpu_ids]
        gen_gpus = []
        for gen_config in self.gen_cfgs:
            gen_gpus += gen_config['gen_gpu_ids']
        all_gpus = train_gpus + gen_gpus
        if not self.gpu_mgr.acquire_many_blocked(self.job_name, all_gpus):
            raise RuntimeError(f"{label} cannot acquire GPUs {all_gpus}")
        print(f"--- {label}: got resources to run init")
        t0 = time.time()
        ...
        gpu_list_str = ','.join([str(i) for i in gen_gpus])
        self.shared_storage.publish("tenant_events", f"{self.job_name}:release_gpu:{gpu_list_str}")
        print(f"--- {label}: init done, resources released")
    ...
    def _run_generation(self, cfg, iteration):
        ...
        self.set_key(f"{self.job_name}_{cfg['name']}_status", "pending")
        gpu_list_str = ','.join([str(i) for i in cfg['gen_gpu_ids']])
        self.shared_storage.publish("tenant_events", f"{self.job_name}:release_gpu:{gpu_list_str}")
