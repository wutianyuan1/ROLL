...
class WeaveScheduler(BaselineScheduler):
    def __init__(self, cost_func: Callable[[Dict], float], max_group_size: int = 5,
        ...
        # Case statistics for add_job
        self.case_stats = {'case1': 0, 'case2': 0, 'case3': 0}
    ...
    def add_job(self, job: Job, timing: datetime = None, double_rollout_resource: bool = False):
        ...
        best_case = None  # Track which case wins
        ...
        if best_case == 1:
            self.case_stats['case1'] += 1
        elif best_case == 2:
            self.case_stats['case2'] += 1
        elif best_case == 3:
            self.case_stats['case3'] += 1
    ...
    def print_case_statistics(self):
        '''
        Print statistics of which case was chosen in add_job calls
        '''
        total = sum(self.case_stats.values())
        if total == 0:
            print("No add_job calls recorded")
            return

        print("\n" + "="*60)
        print("Add Job Case Statistics:")
        print("="*60)
        print(f"Total add_job calls: {total}")
        print(f"Case-1 (share train, share rollout): {self.case_stats['case1']} ({self.case_stats['case1']/total*100:.2f}%)")
        print(f"Case-2 (share train, scale-up rollout): {self.case_stats['case2']} ({self.case_stats['case2']/total*100:.2f}%)")
        print(f"Case-3 (form a new group): {self.case_stats['case3']} ({self.case_stats['case3']/total*100:.2f}%)")
        print("="*60 + "\n")
